...
const StatisticsCode = props =>{
    ...
    const {branchList,findAllBranch} = branchStore
    const {findCommitUserList} = commitsStore
    const {codesStatistics,commitCodeState}=StatisticsStore
    ...
    const [commitCodeData, setCommitCodeData] = useState([])
    ...
    const setStatisticsData = (params, dom) => {
        echarts.dispose(dom)
        codesStatistics(params).then(res => {
            if (res.code === 0) {
                const data = res.data;
                setCommitCodeData(data)
        ...
        <div className='drop-down  statistics'>
            ...
            >
                <div className='statistics-content'>
                    <BreadcrumbContent firstItem={'代码频率'} />
                    <Form
                        name="form"
                        form={form}
                ...
                <div className="statistics-work-content" >
                    {
                        commitCodeData?
                            <div className='statistics-work-text'>
                                <div className='statistics-work-text-nav'>
                                    <span>增加代码行</span>
                                    <span className='statistics-work-text-add'>+{commitCodeData?.addCodeCount}</span>
                                </div>
                                <div className='statistics-work-text-nav'>
                                    <span>减少代码行</span>
                                    <span className='statistics-work-text-delete'>-{commitCodeData?.deleteCodeCount}</span>
                                </div>
                            </div>:
                            <Empty />
                    }
                    <div id="workBar" ref={workBar} style={{ height: "500px", marginTop: "20px" }} >
                        {
                            commitCodeState&&<SpinLoading type="table"/>
                        }
                    </div>
                </div>
            </Col>
...
export default inject('repositoryStore')(observer(StatisticsCode))
